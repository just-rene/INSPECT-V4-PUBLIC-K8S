...
import io.ft.stock_market_data_processing.stock_market_data_processing.events.EventBusOut;
...
public class ComputeDailyCorrelation {
    ...
    public void execute() throws JsonProcessingException {
        System.err.println("computing subscriptions!!!");
        var subscriptions = subscriptionRepository.findAll();
        for (var sub : subscriptions) {
            this.computeForNewTicker(sub.get_id().getTicker());
        }
    }
    ...
    public void computeForNewTicker(String ticker) throws JsonProcessingException {
        ...
        List<StockMarketDayDto> listOfStockMarketDayDtos = eventBusOut.fetchDataForTicker(ticker);
        if (sentimentDayAccumulatedCache.findAll().isEmpty()) {
            System.err.println("no sentiment data found");
        }
        ...
        if (deleteThisEntity != null) {
            stockMarketSentimentCorrelationDailyRepository.delete(deleteThisEntity).block();
        }

        stockMarketSentimentCorrelationDailyRepository.save(stockMarketSentimentCorrelation).block();
    }
